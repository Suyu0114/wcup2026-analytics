...
| **批次 2** | Ch4 EV / bankroll / Kelly；Ch5 EV 計算機逐步操作 | ⏳ 等 **P6 Workstream B** 的 `/value` 定稿再寫 |
...
### Ch4 — 什麼是 EV 和 bankroll？〔批次 2 ⏳〕
`EV = (公平機率 × 賠率) − 1`；bankroll 資金管理；¼ Kelly。
**對齊 P6**：市場模式 ¼ Kelly 不變；**模型模式 Kelly 有校正解鎖閘**（n≥30 且 Brier≤市場×1.1，P6 §3.5）；市場模式判定改**三級 🟢🟡🔴**（P6 §3.2）非二元 value。

### Ch5 — EV 計算機逐步操作〔批次 2 ⏳〕
帶編號步驟對應 UI。**必須對齊 P6 §3.8 的新流程**：選比賽 → **選模式（市場公平價 ⇄ 模型機率〔實驗〕）** → market/outcome（totals 模型模式線格）→ 賠率格式（locale 預設）→ 結果（三級 + 每注 100 + 打平勝率 + Kelly 閘 + 市場參考列）。**不要寫成 P5 舊流程。**
...
批次 2 預計新增：guide Ch4/Ch5 i18n 內容、Ch5 與 P6 `/value` 對齊；想法 #1 的 `/value` searchParams 預填（與 P6 B6 同一機制）。
...
---
guide-spec 建立 2026-06-10（批次 1 已實作 commit `0a8c458`；批次 2 待 P6 B `/value` 定稿）。
