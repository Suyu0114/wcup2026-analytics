...
- 不帶 `--dry-run` = 寫入 Supabase，需 `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` + 已套用 [etl/sql/schema.sql](etl/sql/schema.sql)（**P2 `group_sim` / P3 三物件 / P6 [etl/sql/migrations/p6.sql](etl/sql/migrations/p6.sql) / P8 `group_standings` [etl/sql/migrations/p8.sql](etl/sql/migrations/p8.sql) / P11 `group_scenarios` [etl/sql/migrations/p11.sql](etl/sql/migrations/p11.sql) 需先在 SQL editor 套用**）。
