...
etl/                   ← ingestion + jobs（Elo / fixtures / alias / odds / predict / simulate / calibrate）
  ...
  sql/migrations/p6.sql ← P6 增量 DDL（既有 DB 用這個）
...
- 不帶 `--dry-run` = 寫入 Supabase，需 `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` + 已套用 [etl/sql/schema.sql](etl/sql/schema.sql)（**P2 `group_sim` / P3 三物件 / P6 [etl/sql/migrations/p6.sql](etl/sql/migrations/p6.sql) 需先在 SQL editor 套用**）。
