...
etl/                   ← ingestion + jobs（Elo / fixtures / alias / odds / predict / simulate / calibrate）
  ...
  sql/migrations/p6.sql ← P6 增量 DDL（既有 DB 用這個）；p8.sql ← P8 group_standings
fit/                   ← P6 A2 離線擬合（fit_dc.py → REPORT.md / DIAGNOSIS.md）
sources/               ← adapter（RatingSource / FixtureSource / TheOddsApiSource）
engine/                ← Dixon–Coles 引擎（spec §5）+ group_sim.py（P2 純函數模擬）+ value.py（P3 EV/Kelly）+ standings.py（P8 顯示排名，事實非模型）
web/                   ← P5 Next.js 前端（App Router + Tailwind + next-intl）
  app/[locale]/        ← 頁面（home / matches / results / standings / groups / value）；app/api/* ← server route handlers
...
- 不帶 `--dry-run` = 寫入 Supabase，需 `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` + 已套用 [etl/sql/schema.sql](etl/sql/schema.sql)（**P2 `group_sim` / P3 三物件 / P6 [etl/sql/migrations/p6.sql](etl/sql/migrations/p6.sql) / P8 `group_standings` [etl/sql/migrations/p8.sql](etl/sql/migrations/p8.sql) 需先在 SQL editor 套用**）。
